...
mongoose.connect("mongodb://0.0.0.0/DEBTInFO");
...
const comData = mongoose.model("comData", commoditySchema);
...
const items = mongoose.model("items", comSchema);
...
app.get("/commodities", async (req, res) => {
  const commodities = await items.find();
  res.render("commodities", { commodities: commodities });
});
...
app.post("/submitCommodityForm", (req, res) => {
  const { commodity, price, state } = req.body;
